...
function App() {
  ...
  const professionalExperience = [
    {
      ...
      subtitle: "Jagoan Hosting Infra Comp.",
      date: "Jun - Nov 2025",
      icon: "server-line",
    },
    {
  ...
  return (
    <div className={`min-h-screen w-full transition-colors duration-500 overflow-hidden relative selection:bg-white selection:text-black ${isDarkTheme ? "bg-[#050505] text-white" : "bg-[#fafafa] text-black"}`}>
      ...
      <div className="fixed top-0 left-0 w-full h-screen overflow-hidden z-0 pointer-events-none">
        <div className={`absolute inset-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_100%,#000_50%,transparent_100%)] ${!isDarkTheme && "opacity-40"}`}></div>
      </div>
      {/* ========================================================= */}
      ...
      <div className="relative z-20 w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-8">
        ...
        {/* --- 1. HERO SECTION --- */}
        <section id="hero" className="min-h-screen flex flex-col justify-center pt-32 pb-10 scroll-mt-28">
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-center">
            <motion.div variants={stagger} initial="hidden" animate="visible" className="lg:col-span-7 flex flex-col order-2 lg:order-1">
              <LiveClock isDarkTheme={isDarkTheme} />

              <motion.p variants={fadeUp} className={`text-sm md:text-base font-semibold tracking-[0.2em] uppercase mb-4 ${isDarkTheme ? "text-zinc-400" : "text-zinc-500"}`}>
                Developer
              </motion.p>
              <motion.h1 variants={fadeUp} className="text-[3.2rem] leading-[1.05] sm:text-7xl md:text-8xl lg:text-[6rem] font-black tracking-tighter uppercase mb-4">
                Menciptakan <br /> Prototipe <br /> Digital
              </motion.h1>
              <motion.p variants={fadeUp} className={`text-base md:text-lg max-w-lg leading-relaxed ${isDarkTheme ? "text-zinc-400" : "text-zinc-600"}`}>
        ...
        {/* --- 2. PROFILE SECTION --- */}
        <section id="about" className={`py-24 border-t scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8 }} className="lg:col-span-4 flex justify-center lg:justify-start">
              <div className={`w-full max-w-[320px] rounded-[2.5rem] p-6 md:p-8 flex flex-col items-center text-center border shadow-2xl ${isDarkTheme ? "bg-[#111111] border-white/10" : "bg-white border-black/10"}`}>
                ...
                <p className={`text-[10px] font-black tracking-widest uppercase mb-5 ${isDarkTheme ? "text-zinc-500" : "text-zinc-400"}`}>
                ...
                <div className="flex gap-4">
                  {["github-line", "linkedin-line", "instagram-line"].map((icon, i) => (
                    <a key={i} href="#" className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all hover:scale-110 hover:-translate-y-1 ${isDarkTheme ? "border-white/10 hover:bg-white/10 text-white" : "border-black/10 hover:bg-black/5 text-black"}`}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8, delay: 0.2 }} className="lg:col-span-8 flex flex-col gap-6">
        ...
        <section id="experience" className={`py-24 border-t relative overflow-hidden scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
          <div className="grid lg:grid-cols-12 gap-16 lg:gap-8 relative">
            <div className="lg:col-span-5 relative">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }} className="lg:sticky lg:top-40 flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-4">
                    {displayedEducation.map((item, index) => (
                      <motion.div key={item.id} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1, duration: 0.5 }} className="relative flex items-center group pl-12 md:pl-[84px] overflow-hidden">
                        ...
                        <div className={`w-full flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-[1.5rem] border transition-all duration-300 hover:-translate-y-1 ${isDarkTheme ? "bg-[#111] border-white/5 hover:border-white/20" : "bg-white border-black/5 hover:border-black/20 hover:shadow-lg"}`}>
                    ...
                    <AnimatePresence initial={false}>
                      {displayedExperience.map((item, index) => (
                        <motion.div key={item.id} initial={{ opacity: 0, height: 0, y: -20 }} animate={{ opacity: 1, height: "auto", y: 0 }} exit={{ opacity: 0, height: 0, y: -20 }} transition={{ duration: 0.3 }} className="relative flex items-center group pl-12 md:pl-[84px] origin-top overflow-hidden">
                          ...
                          <div className={`absolute left-[20px] md:left-[40px] w-8 md:w-11 h-[2px] transition-colors duration-300 ${isDarkTheme ? "bg-white/5 group-hover:bg-white/20" : "bg-black/5 group-hover:bg-black/20"}`}></div>
                          <div className={`w-full flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 rounded-[1.5rem] border transition-all duration-300 hover:-translate-y-1 ${isDarkTheme ? "bg-[#111] border-white/5 hover:border-blue-500/30" : "bg-white border-black/5 hover:border-blue-500/30 hover:shadow-lg"}`}>
                            <div className="flex items-center gap-4">
        ...
        <section id="highlights" className={`py-24 border-t relative scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            {/* Github */}
            <a href="https://github.com/username-github-anda" target="_blank" rel="noopener noreferrer" className={`col-span-1 md:col-span-12 lg:col-span-6 relative overflow-hidden p-6 md:p-8 rounded-[2rem] border flex flex-col justify-between group transition-all duration-300 hover:-translate-y-1 block cursor-pointer ${isDarkTheme ? "bg-[#111]/80 border-white/5 hover:border-white/20" : "bg-white border-black/5 hover:border-black/20"}`}>
              ...
              <div className="grid grid-cols-12 gap-1.5 opacity-80 mb-4">
                {Array.from({ length: 36 }).map((_, i) => {
                  ...
                  return (
                    <motion.div key={i} animate={isGreen || isPurple ? { opacity: [baseOpacity, baseOpacity * 0.3, baseOpacity] } : { opacity: isDarkTheme ? 0.1 : 0.4 }} transition={{ duration: Math.random() * 3 + 2, repeat: Infinity, ease: "easeInOut", delay: Math.random() * 2 }} whileHover={{ scale: 1.5, zIndex: 10, opacity: 1 }} className={`w-full aspect-square rounded-[3px] transition-colors ${isPurple ? "bg-violet-500" : isGreen ? "bg-green-500" : isDarkTheme ? "bg-zinc-800" : "bg-zinc-300"}`} />
                  );
                })}
              </div>
            ...
            {/* Letterboxd */}
            <div className={`col-span-1 md:col-span-12 lg:col-span-7 relative overflow-hidden p-6 md:p-8 rounded-[2rem] border flex flex-col group transition-all duration-300 hover:-translate-y-1 ${isDarkTheme ? "bg-[#111]/80 border-white/5 hover:border-orange-500/30" : "bg-white border-black/5 hover:border-orange-500/30"}`}>
              <div className="flex justify-between items-center mb-6">
              ...
              <div className="grid grid-cols-4 gap-3 md:gap-4 mt-auto">
                {["/assets/poster/poster1.jpg", "/assets/poster/poster2.jpg", "/assets/poster/poster3.jpg", "/assets/poster/poster4.jpg"].map((poster, i) => (
                  <div key={i} className="relative aspect-[2/3] rounded-xl overflow-hidden shadow-lg border border-white/10 group/poster">
                    <img src={poster} alt={`Movie Poster ${i + 1}`} className="w-full h-full object-cover transition-transform duration-500 group-hover/poster:scale-110" />
            ...
            {/* Learning */}
            <div className={`col-span-1 md:col-span-12 lg:col-span-6 relative overflow-hidden p-6 md:p-8 rounded-[2rem] border flex flex-col group transition-all duration-300 hover:-translate-y-1 ${isDarkTheme ? "bg-[#111]/80 border-white/5" : "bg-white border-black/5"}`}>
        ...
        <section id="tools" className={`py-24 border-t relative scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }} className="relative z-10">
            <div className="text-center mb-16">
              <motion.h2 variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-4xl md:text-5xl font-black mb-4 tracking-tight">
                {t("toolsTitle") || "Tools & Skills"}
              </motion.h2>
              <motion.p variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className={`text-base md:text-lg ${isDarkTheme ? "text-zinc-400" : "text-zinc-600"}`}>
                {t("toolsDesc") || "Teknologi yang saya gunakan untuk menciptakan ekosistem digital."}
              </motion.p>
            </div>
              ...
              {listTools.map((tool) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center text-center relative z-10">
                    <h4 className="font-bold text-sm tracking-wide leading-tight line-clamp-1">{tool.nama}</h4>
                    <p className={`text-[9px] font-bold tracking-widest uppercase mt-1.5 ${isDarkTheme ? "text-zinc-500" : "text-zinc-400"}`}>
                      {tool.ket}
                    </p>
                  </div>
                </motion.div>
            ...
            <div className="flex items-center justify-center gap-2 mt-2 md:hidden text-[10px] font-bold tracking-widest uppercase text-zinc-500">
              <i className="ri-arrow-left-line animate-pulse"></i> Geser Tools <i className="ri-arrow-right-line animate-pulse"></i>
            </div>
          </motion.div>
        </section>

        {/* --- 6. PROJECTS SECTION (INTERAKTIF SCROLL HORIZONTAL) --- */}
        <section id="projects" className={`py-24 border-t relative overflow-hidden scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
        ...
        {/* ========================================= */}
        <section id="contact" className={`py-32 relative overflow-hidden border-t scroll-mt-28 ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
          ...
          <div className="relative z-10 w-full max-w-6xl mx-auto px-6 md:px-12 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-start">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={stagger} className="flex flex-col gap-8">
                ...
                <motion.div variants={fadeUp} className="flex flex-col gap-6 mt-4">
                  <div className={`group flex items-center gap-6 pb-6 border-b ${isDarkTheme ? "border-white/10" : "border-black/10"}`}>
                    <div className={`w-14 h-14 shrink-0 rounded-full flex items-center justify-center border transition-all duration-300 ${isDarkTheme ? "bg-[#111] border-white/10 group-hover:border-white/50" : "bg-white border-black/10 shadow-sm"}`}>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.8, ease: "easeOut" }}>
                <div className={`w-full p-8 md:p-10 rounded-[2.5rem] border shadow-2xl relative ${isDarkTheme ? "bg-[#111] border-white/10" : "bg-white border-black/10"}`}>
                  
                  <div className={`flex p-1.5 rounded-2xl mb-8 ${isDarkTheme ? "bg-[#0a0a0a] border border-white/5" : "bg-zinc-100 border border-black/5"}`}>
                  ...
                  <form className="flex flex-col gap-6" onSubmit={contactMethod === "wa" ? handleWhatsAppSubmit : (e) => e.preventDefault()}>
                    <div className="flex flex-col gap-2">
                      ...
                      <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="John Doe" className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all text-sm font-medium ${isDarkTheme ? "bg-[#0a0a0a] border-white/10 focus:border-white/30 text-white placeholder-zinc-700" : "bg-zinc-50 border-black/10 focus:border-black/30 text-black placeholder-zinc-400"}`} />
                    </div>
                    ...
                    <AnimatePresence>
                      {contactMethod === "email" && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-2 overflow-hidden">
                          ...
                          <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="john@example.com" className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all text-sm font-medium ${isDarkTheme ? "bg-[#0a0a0a] border-white/10 focus:border-white/30 text-white placeholder-zinc-700" : "bg-zinc-50 border-black/10 focus:border-black/30 text-black placeholder-zinc-400"}`} />
                        </motion.div>
                      )}
                    </AnimatePresence>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative">
                        <select value={formData.interest} onChange={(e) => setFormData({...formData, interest: e.target.value})} className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all text-sm font-medium appearance-none cursor-pointer ${isDarkTheme ? "bg-[#0a0a0a] border-white/10 focus:border-white/30 text-white" : "bg-zinc-50 border-black/10 focus:border-black/30 text-black"}`}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <textarea rows="3" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} placeholder="Tell me about your project, timeline, or just say hello..." className={`w-full px-5 py-4 rounded-2xl border outline-none transition-all text-sm font-medium resize-none ${isDarkTheme ? "bg-[#0a0a0a] border-white/10 focus:border-white/30 text-white placeholder-zinc-700" : "bg-zinc-50 border-black/10 focus:border-black/30 text-black placeholder-zinc-400"}`}></textarea>
                    </div>
      ...
      {/* 8. FOOTER SECTION                           */}
      {/* ========================================= */}
      <footer className={`relative z-20 pt-24 pb-8 md:pb-12 overflow-hidden ${isDarkTheme ? "bg-[#0a0a0a]" : "bg-zinc-100"}`}>
        ...
        <div className="w-full max-w-[1600px] mx-auto px-6 md:px-12 lg:px-8 flex flex-col gap-20">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-4">
            <div className="flex flex-col items-center md:items-start gap-1.5 w-full md:w-1/3">
              <p className={`text-[10px] font-medium tracking-widest uppercase flex items-center gap-2 ${isDarkTheme ? "text-zinc-600" : "text-zinc-400"}`}>
            ...
            <div className="flex flex-col items-center md:items-end gap-3 w-full md:w-1/3">
              <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className={`group flex items-center gap-2 text-[10px] font-bold tracking-widest uppercase transition-colors ${isDarkTheme ? "text-zinc-400 hover:text-white" : "text-zinc-500 hover:text-black"}`}>
                Kembali ke Atas{" "}
                <div className={`w-6 h-6 rounded-full flex items-center justify-center border transition-all group-hover:-translate-y-1 ${isDarkTheme ? "border-white/20 bg-white/10 text-white" : "border-black/20 bg-black/10 text-black"}`}>
